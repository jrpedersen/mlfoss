...
def resnext50_32x4d_foss(pretrained=False, progress=False, **kwargs):
    r"""ResNeXt-50 32x4d model from
    `"Aggregated Residual Transformation for Deep Neural Networks" <https://arxiv.org/pdf/1611.05431.pdf>`_
    ...
    kwargs['groups'] = 32
    kwargs['width_per_group'] = 4
    return _resnet('resnext50_32x4d_foss', Bottleneck, [3, 4, 6, 3],
                   pretrained, progress, **kwargs)
...
class ConvNet(nn.Module):
    def __init__(self, picture_dim, target_dim,num_inputs, num_channels, kernel_size, dropout=0.2): #, kernel_size=3
        ...
        for i in range(num_levels):
            ...
            layers += [Block(in_channels, out_channels, out_channels_final,kernel_size[i], stride=1,
                                     padding=int((kernel_size[i]-1)/2), dropout=dropout)]
