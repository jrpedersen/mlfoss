...
#Gather FO's
if config.da_addafo:
    fo= AFO.gather_fos(current_location+'/data/'+config.dataset[:-8]+'_globalref_unnorm_'+str(config.n_input_ch)+'ch.hdf5',mix=(config.dataset[:3]=='MIX'))
else:
    fo=[]
    ...
    for epoch in range(1, config.N_EPHOCS+1):
        ...
        if config.save:
            torch.save({
                ...
                'experiments/'+experiment_name+'/ConvNet_k'+str(ksplit)+'_e'+str(epoch)+'.tar')
    #writer.close()
